"""
Test function for trader-joe-chip-shopping.

Evaluates the total checkout amount.
"""

import json
import re

# Ground truth value (note: evaluation.md shows 56.36 without $, but instruction says to include $)
EXPECTED_ANSWER = 56.36
...
def _parse_float(answer: str) -> float | None:
    """Parse float from answer string (handles $ symbol)."""
    try:
        # Remove any whitespace, dollar signs, and try to parse as float
...
def test(result: dict) -> dict:
    ...
    if not output_answer:
        return {
            "passed": False,
            "feedback": "No valid answer found in assistant responses. Expected format: <answer>$56.36</answer>",
            "details": {
    ...
    # Parse float from answer
    output_num = _parse_float(output_answer)

    if output_num is None:
        return {
            "passed": False,
            "feedback": f"Could not parse answer as number: {output_answer}",
            "details": {
                "task_completed": task_completed,
    ...
    answer_correct = abs(output_num - EXPECTED_ANSWER) < 0.01
    ...
    feedback_parts.append(
        f"{'✓' if answer_correct else '✗'} Total amount: got {output_num}, expected {EXPECTED_ANSWER}."
    )
