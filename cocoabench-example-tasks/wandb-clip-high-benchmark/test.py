"""
Test function for wandb-clip-high-benchmark.

Evaluates the number of validation benchmarks with clear gain.
"""
...
def _parse_integer(answer: str) -> int | None:
    """Parse integer from answer string."""
    try:
        # Remove any whitespace and try to parse as int
...
def test(result: dict) -> dict:
    ...
    if not output_answer:
        return {
            "passed": False,
            "feedback": "No valid answer found in assistant responses. Expected format: <answer>3</answer>",
            "details": {
    ...
    # Parse integer from answer
    output_num = _parse_integer(output_answer)

    if output_num is None:
        return {
            "passed": False,
            "feedback": f"Could not parse answer as integer: {output_answer}",
            "details": {
                "task_completed": task_completed,
    ...
    answer_correct = output_num == EXPECTED_ANSWER
    ...
    feedback_parts.append(
        f"{'✓' if answer_correct else '✗'} Number of benchmarks: got {output_num}, expected {EXPECTED_ANSWER}."
    )
